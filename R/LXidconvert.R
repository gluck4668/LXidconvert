...
com_packs <- c("openxlsx","tidyr","purrr","httr","RSQLite","reshape2",
               ...
               "Biobase","IRanges","S4Vectors","devtools","tidyverse","readr")
...
if(file_type=="txt")
  df <- read_table(data_file) else
  df <- eval(str2expression(paste0("read.",file_type,"(data_file)")))
...
if( tolower(trimws(data_type))=="gene")
  ...
  id_convert <- tryCatch(
                   ...
                   error=function(e){stop("There are no matching IDs.The 'species' or 'from_id' may be wrong. Please check it.")}
                   )

  if(any(colnames(id_convert)=="SYMBOL"))
    colnames(id_convert)[grep("SYMBOL",colnames(id_convert))] <- c("Gene_Symbol")
...
id_convert <- tryCatch(
  CTSgetR(id = df$from_id,from = trimws(from_id),to = trimws(to_id) ) %>% data.frame(),
  error=function(e){stop("There are no matching IDs.The 'data_type' may be wrong. Please check it.")}
)
